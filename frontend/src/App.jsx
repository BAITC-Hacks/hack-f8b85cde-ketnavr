...
function DetailDialog({ row, onClose, mode }) {
  ...
  return <dialog className="detail-dialog" ref={ref} onClose={onClose} aria-labelledby="detail-title" onClick={(event) => { if (event.target === event.currentTarget) ref.current.close(); }}>
    ...
    <section className="reason-panel"><div><CircleHelp size={18} /><h3>Почему такой заказ</h3></div><p>{row.reason}</p></section>
    <dl className="detail-facts">{facts.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>
    <div className="detail-total"><div><span>Рекомендовано к заказу</span><strong>{number(row.recommended_order_qty)} <small>{row.unit}</small></strong></div><div className="detail-price"><span>Сумма по переданной цене</span><strong>{row.unit_price_kzt === null ? 'Цена не указана' : money(row.recommended_order_qty * row.unit_price_kzt)}</strong></div></div>
...
export default function App() {
  ...
  const visibleRows = useMemo(() => sortRows(filterRows(rows, filters), sort), [data, filters, sort]);
    ...
    <div className="main-shell">
      ...
      <main id="recommendations">
        ...
        <section className="metrics" aria-label="Сводка по текущей выборке">
          <Metric icon={Boxes} label="Товарных позиций" value={available ? number(summary.positions) : '—'} note={hasFilters ? 'С учётом выбранных фильтров' : 'В текущем расчёте'} />
